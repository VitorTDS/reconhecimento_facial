...
# Usa apenas um VideoCapture para evitar conflito de acesso à webcam.
...
def processar_frames():
    ...
    while True:
        with lock:
            frame_local = None if ultimo_frame is None else ultimo_frame.copy()
        ...
        try:
            pequeno = cv2.resize(frame_local, (0, 0), fx=0.5, fy=0.5)
            ...
            img_h, img_w = frame_local.shape[:2]
            novas_pessoas = []

            for (x, y, w, h) in rostos:
                x2, y2, w2, h2 = x * 2, y * 2, w * 2, h * 2
                rosto_crop = frame_local[y2 : y2 + h2, x2 : x2 + w2]
...
def trocar_camera():
    global cam, indice_atual, ultimo_frame

    novo = 1 if indice_atual == 0 else 0
    nova_camera = cv2.VideoCapture(novo)
    if nova_camera.isOpened():
        cam.release()
        cam = nova_camera
        indice_atual = novo
        with lock:
